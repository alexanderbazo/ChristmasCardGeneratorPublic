...
function createViewFromFrame(context, frame) {
  ...
  frameEl.querySelector(`.time-selector [data-delay="${frame.delay}"]`).classList.add("selected");
  frameEl.querySelector(".trash").addEventListener("click", onRemoveFrameButtonClicked.bind(context, frame.id));
  frameEl.querySelector(".time-selector").addEventListener("click", onFrameDelaySelected.bind(context, frame.id));
  return frameEl;
}
...
class TimelineView extends Observable {
  ...
  updateFrame(frame) {
    ...
    frameEl.querySelector("img").src = frame.image.src;
    frameEl.querySelector(".time-selector .selected").classList.remove("selected");
    frameEl.querySelector(`.time-selector [data-delay="${frame.delay}"]`).classList.add("selected");
  } 
